...
async function session():Promise<Session|null>{
  ...
  const raw=u as any,profile=raw.userMetadata||raw.user_metadata||{},email=String(raw.email||'').toLowerCase();if(!email)return null
...
async function adminSave(req:Request,s:Session,resource:string,id?:string){if(s.role!=='admin')return json({error:'Brak uprawnień administratora'},403);const c=resources[resource as Resource];if(!c)return json({error:'Nieprawidłowy moduł'},404);const body=await req.json()as Record<string,unknown>,fields=adminFields(body,c);if(!id&&!String(body[c.required]||'').trim())return json({error:'Uzupełnij wymagane pole'},400);const row=id?await update(c.table,id,fields):await create(c.table,fields);return json({row:{id:row.id}},id?200:201)}
